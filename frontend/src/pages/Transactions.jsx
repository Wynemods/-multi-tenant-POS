          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {loading ? (
              ...
              transactions.map((transaction, index) => {
                ...
                const paymentMethod = transaction.paymentMethod ?? transaction.payment_method ?? 'N/A'
                ...
                return (
                  <tr key={transaction.id} className={`hover:bg-gray-50 transition-colors ${index % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                    <td className="px-4 py-2 whitespace-nowrap border-r border-gray-200">
                      <span className="font-mono font-semibold text-sm text-primary-600">{transactionNumber}</span>
                    </td>
                    ...
                    <td className="px-4 py-2 whitespace-nowrap border-r border-gray-200">
                      <div className="flex items-center gap-2">
                        <span className="text-gray-500">{getPaymentIcon(paymentMethod)}</span>
                        <span className="text-sm font-medium capitalize text-gray-700">{paymentMethod}</span>
                      </div>
                    </td>
                    <td className="px-4 py-2 whitespace-nowrap border-r border-gray-200">
            ...
            <div className="bg-gradient-to-r from-primary-600 to-accent-600 px-6 py-4 text-white">
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <p className="text-sm text-white/90 mt-1">Transaction #{selectedTransaction.transactionNumber ?? selectedTransaction.transaction_number ?? 'N/A'}</p>
                </div>
                <button
            ...
            <div className="flex-1 overflow-y-auto p-6">
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-4 pb-4 border-b border-gray-200">
                  <div>
                    ...
                    <p className="text-sm font-semibold text-gray-900">
                      {(() => {
                        const date = selectedTransaction.createdAt ?? selectedTransaction.created_at
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2">
                      {(() => {
                        const paymentMethod = selectedTransaction.paymentMethod ?? selectedTransaction.payment_method ?? 'N/A'
                        return (
                          <>
                            <span className="text-gray-500">{getPaymentIcon(paymentMethod)}</span>
                            <p className="text-sm font-semibold text-gray-900 capitalize">{paymentMethod}</p>
                          </>
                        )
                      })()}
                    </div>
                  ...
                  <div className="border border-gray-200 rounded-lg overflow-hidden">
                    ...
                    <div className="divide-y divide-gray-200">
                      {selectedTransaction.items?.map((item, index) => {
                        ...
                        const itemType = item.itemType ?? item.item_type ?? item.type ?? 'product'
                        ...
                        <div key={index} className="px-4 py-3 grid grid-cols-12 gap-2 hover:bg-gray-50 transition-colors">
                          ...
                          <div className="col-span-2 text-right">
                            <span className="text-sm text-gray-600">{formatCurrency(unitPrice)}</span>
                          </div>
                          <div className="col-span-2 text-right">
                ...
                <div className="bg-gradient-to-r from-gray-50 to-gray-100 rounded-lg p-4 border border-gray-200">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-2xl font-bold text-primary-600">
                      {formatCurrency(selectedTransaction.totalAmount ?? selectedTransaction.total_amount ?? 0)}
                    </span>
                  </div>
                </div>
                ...
                <div className="flex justify-center pt-2">
                  {(() => {
                    const status = selectedTransaction.status ?? 'completed'
                    return (
